...
public:
    ...
    bool CheckSubBox(int row, int col, int number); // 5 points, returns true if the number exists
    int  SolvePuzzle(int iterations); // 10 points, iterates through the puzzle based on the parameter iterations
    ...
    void printSubBox(int row, int col); // checks CheckSubBox
    bool checkZero(); // checks matrix for zero
    virtual ~mySudoku();
private:
    int Data[Size][Size];
};
